...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            self.dependencies.append((self.stack[l-1],self.stack[l-2]))
            self.stack.pop(l-2)
        elif transition == "RA":
            self.dependencies.append((self.stack[l-2],self.stack[l-1]))
            self.stack.pop(l-1)
...
def minibatch_parse(sentences, model, batch_size):
    ...
    # print(str(partial_parses[0].sentence))
    unfinished_parses = partial_parses[:]
    while len(unfinished_parses)!=0:
        transitions = model.predict(unfinished_parses[:batch_size])
        # print(str(len(transitions)-batch_size))
        for trans,part in zip(transitions,unfinished_parses[:batch_size]):
            part.parse_step(trans)
            l1=len(part.stack)
            l2=len(part.buffer)
            if l1==1 and l2==0:
                # print("HELLO")
                unfinished_parses.remove(part)
